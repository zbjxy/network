 ...
 *
 * @version 2.10 26.08.2003 computeRoute returns boolean result, setXXXNode
 *          added, modified reporting
 ...
 * @version 1.20 30.04.2000 decreaseUsage corrected, traversed nodes and their
 *          degree counted
...
public class MovingObject {
	...
	private boolean computeNextPoint(int newTime, Reporter reporter) {
		...
		while (true) {
			...
			if (relDist + maxDistOnEdge < actDist) {
				...
				//System.out.println("previous x,y " + llx + " " + lly
						//+ " last xy: " + lastX + " " + lastY);
				int oldRepNum = repNum;
				Node nextNode = route.getDestinationNode();
				repNum = reporter.reportMovingObject(newTime, id, repNum,
						objClass, lastX, lastY, speed, doneDist,
						nextNode.getX(), nextNode.getY(),
						objClasses.getReportProbability(objClass));
				if (repNum != oldRepNum)
					doneDist = 0;
				util.Timer.start(1);
				return false;
			...
			// case 2: next node is reached
				...
				doneDist += computeDistance(llx, lly, lastNode.getX(),
						lastNode.getY());
				llx = lastNode.getX();
				lly = lastNode.getY();
				...
				// report
				util.Timer.stop(1);
				reporter.reportEdge(newTime - remainingTime, id, ++edgeNum,
						objClass, actEdge.getId(), actEdge.getEdgeClass(),
						route.getStartingNode().getX(), route.getStartingNode()
								.getY(), speed, route.getDestinationNode()
								.getX(), route.getDestinationNode().getY(),
						objClasses.getReportProbability(objClass));
				util.Timer.start(1);
				// if significant speed change then re-route
				if (container.getReRoute().computeNewRouteByComparison(
						lastTime, actTime,
						(int) (0x7fffffff / route.getOrigWeight()),
						(int) (0x7fffffff / actWeight))) {
					reroute(actEdge);
				}
			}
		}
	 ...
	 * 
	 * @param x1
	 *            x1-coordinate of the edge
	 * @param y1
	 *            y1-coordinate of the edge
	 * @param x2
	 *            x2-coordinate of the edge
	 * @param y2
	 *            y2-coordinate of the edge
	 * @param portion
	 *            portion of the edge
	 * @param length
	 *            distance between (x1,y1) and (x2,y2)
	 */
	// Liu: change this .x1y1 x2y2 are just 2 end points, and now we will just
	// need to add a width to reflect this change.
	// Think of x1y1 x2y2 as a straight line, we just need to tweak a little bit
	// on this. Suppose for all street the width is 10.
	private void computePoint(int x1, int y1, int x2, int y2, double portion,
			double length) {
		...
		try {
		    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("/Users/leoliu/workspace/network/data/output2.txt", true)));
		    out.println(x1+" "+y1+" "+x2+" "+y2+" "+lastX+" "+lastY);
		    out.close();
		} catch (IOException e) {
	 ...
	 *            the edge
	 * @param start
	 *            the node which the object traversed last
	 * @param portion
	 *            distance from start
	 */
	private void computePoint(Edge edge, Node start, double portion) {
		if (start == edge.getNode1())
			computePoint(edge.getNode1().getX(), edge.getNode1().getY(), edge
					.getNode2().getX(), edge.getNode2().getY(), portion,
					edge.getLength());
		else
			computePoint(edge.getNode2().getX(), edge.getNode2().getY(), edge
					.getNode1().getX(), edge.getNode1().getY(), portion,
					edge.getLength());
	}

	/**
	 * Computes a new route from the current position of the object to its
	 * destination.
	 * 
	 ...
	 * 
	 * @param actPathEdge
	 *            the path edge which stops the operation
	 */
	...
	/**
	 * Moves the object to the position at the new time. The position(s) are
	 * reported.
	 * 
	...
	public void reportEnd(Reporter reporter) {
		util.Timer.stop(1);
		if (reporter != null) {
			reporter.reportDisappearingObject(arrivalTime, id, repNum,
					objClass, dest.getX(), dest.getY(), doneDist,
					objClasses.getReportProbability(objClass));
		}
		util.Timer.start(1);
	}
	...
	public void reportNewObject(Reporter reporter) {
		...
		if (reporter != null) {
			...
			double speed = actDist / actWeight;
			Node nextNode = route.getDestinationNode();
			repNum = reporter.reportNewMovingObject(startTime, id, objClass,
					start.getX(), start.getY(), speed, nextNode.getX(),
					nextNode.getY(), objClasses.getReportProbability(objClass));
			reporter.reportEdge(startTime, id, ++edgeNum, objClass, currEdge
					.getId(), currEdge.getEdgeClass(), route.getStartingNode()
					.getX(), route.getStartingNode().getY(), speed, route
					.getDestinationNode().getX(), route.getDestinationNode()
					.getY(), objClasses.getReportProbability(objClass));
		}
		util.Timer.start(1);
	}
	...
	private void reroute(Edge actEdge) {
		container.getWeightManager().setActualObjectClass(objClass);
		util.Timer.start(2);
		PathEdge newPath = container.getNetwork().computeFastWay2(
				actEdge.getOppositeNode(lastNode), dest);
		util.Timer.stop(2);
