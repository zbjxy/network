...
public abstract class DataGenerator extends ShowNetworkMap implements
	...
	public static void main(String nameOfApplet) {
		...
		try {
			...
			aDataGenerator = (DataGenerator) java.beans.Beans.instantiate(
					iiClsLoader, nameOfApplet);
			...
			System.err
					.println("Exception occurred in main() of generator2.DataGenerator "
							+ exception);
	...
	protected void addComponentsToApplet() {
		...
		add(getNumExtObjClassesTextField(), getNumExtObjClassesTextField()
				.getName());
	...
	public void adjustmentValueChanged(java.awt.event.AdjustmentEvent e) {
		if ((e.getSource() == getTimeScrollbar()) && (time != null)) {
			int newTime = e.getValue()
					* time.getMaxTime()
					/ (timeScrollbar.getMaximum() - timeScrollbar
							.getVisibleAmount());
			if (newTime != actTime)
				;
			{
	...
	public void changeComponentPositions() {
		...
		getMaxTimeTextField().setBounds(viewX + 150, viewY + viewHeight + 82,
				40, 23);
		getNumObjClassesLabel().setBounds(viewX + viewWidth / 2 + 25,
				viewY + viewHeight + 82, 155, 23);
		getNumObjClassesTextField().setBounds(viewX + viewWidth / 2 + 185,
				viewY + viewHeight + 82, 30, 23);
		getNumExtObjClassesTextField().setBounds(viewX + viewWidth / 2 + 220,
				viewY + viewHeight + 82, 30, 23);
		getObjBeginLabel().setBounds(viewX + viewWidth / 2 + 25,
				viewY + viewHeight + 1, 155, 23);
		getObjBeginTextField().setBounds(viewX + viewWidth / 2 + 185,
				viewY + viewHeight + 1, 30, 21);
		getExtObjBeginTextField().setBounds(viewX + viewWidth / 2 + 220,
				viewY + viewHeight + 1, 30, 21);
		getObjPerTimeLabel().setBounds(viewX + viewWidth / 2 + 25,
				viewY + viewHeight + 23, 155, 23);
		getObjPerTimeTextField().setBounds(viewX + viewWidth / 2 + 185,
				viewY + viewHeight + 23, 30, 21);
		getExtObjPerTimeTextField().setBounds(viewX + viewWidth / 2 + 220,
				viewY + viewHeight + 23, 30, 21);
		getCopyrightLabel().setBounds(viewX, viewY + viewHeight + 180,
				viewWidth - 100, 19);
		getReportProbLabel()
				.setBounds(viewX, viewY + viewHeight + 112, 148, 23);
		getReportProbTextField().setBounds(viewX + 150,
				viewY + viewHeight + 112, 40, 23);
		getMsdLabel().setBounds(viewX + 195, viewY + viewHeight + 112, 260, 23);
		getMsdTextField().setBounds(viewX + viewWidth / 2 + 220,
				viewY + viewHeight + 112, 30, 23);
		getDeleteButton()
				.setBounds(viewX + 150, viewY + viewHeight + 6, 75, 29);
	}
	...
	public synchronized void compute() {
		...
		if (time == null) {
			time = new Time(properties, getValueOfTextField(
					getMaxTimeTextField(), MIN_MAXTIME, MAX_MAXTIME, false));
			dataspace = new DataSpace(drawableObjects);
			objClasses = createObjectClasses(
					properties,
					time,
					dataspace,
					getValueOfTextField(getNumObjClassesTextField(), 1,
							MAX_OBJCLASSES, false),
					getValueOfTextField(getReportProbTextField(), 0, 1000,
							false),
					getValueOfTextField(getMsdTextField(), 1, 1000, true));
			extObjClasses = createExternalObjectClasses(
					properties,
					time,
					dataspace,
					getValueOfTextField(getNumExtObjClassesTextField(), 1,
		...
		ExternalObjectGenerator extObjGen = createExternalObjectGenerator(
				...
				dataspace,
				extObjClasses,
				getValueOfTextField(getExtObjPerTimeTextField(), 0,
						MAX_EXTOBJPERTIME, true),
				getValueOfTextField(getExtObjBeginTextField(), 0,
						MAX_EXTOBJBEGIN, true));
		...
		ObjectGenerator objGen = createObjectGenerator(
				...
				nodes,
				objClasses,
				getValueOfTextField(getObjPerTimeTextField(), 0,
						MAX_OBJPERTIME, true),
				getValueOfTextField(getObjBeginTextField(), 0, MAX_OBJBEGIN,
						true));
	...
	protected Label getMaxTimeLabel() {
		if (maxTimeLabel == null) {
			...
			maxTimeLabel.setText("maximum time (" + MIN_MAXTIME + "-"
					+ MAX_MAXTIME + "):");
		}
		;
		return maxTimeLabel;
	...
	protected Label getNumObjClassesLabel() {
		if (numObjClassesLabel == null) {
			...
			numObjClassesLabel.setText("classes (M:1-" + MAX_OBJCLASSES
					+ "/E:1-" + MAX_EXTOBJCLASSES + "):");
		}
		;
	...
	protected Label getObjBeginLabel() {
		if (objBeginLabel == null) {
			...
			objBeginLabel.setText("obj./begin (M:-" + MAX_OBJBEGIN + " E:-"
					+ MAX_EXTOBJBEGIN + "):");
		}
		;
	...
	protected Label getObjPerTimeLabel() {
		if (objPerTimeLabel == null) {
			...
			objPerTimeLabel.setText("obj./time (M:-" + MAX_OBJPERTIME + "/E:-"
					+ MAX_EXTOBJPERTIME + "):");
		}
		;
	...
	protected Scrollbar getTimeScrollbar() {
		if (timeScrollbar == null) {
			timeScrollbar = new Scrollbar(Scrollbar.HORIZONTAL, 0, 32, 0,
					viewWidth - 32);
	 ...
	 */
	protected int getValueOfTextField(TextField tf, int min, int max,
	...
	protected void setState(int state) {
		...
		if (getValueOfTextField(getMaxTimeTextField(), MIN_MAXTIME,
				MAX_MAXTIME, true) != 20)
			compute();
		// Compute button
		getComputeButton().setEnabled(true);
	...
	protected void setTimeScrollbar(int t) {
		int value = t
				* (timeScrollbar.getMaximum() - timeScrollbar
						.getVisibleAmount()) / time.getMaxTime();
