...
public class ExternalObjectClasses {
	 ...
	 */
	public ExternalObjectClasses(Properties properties, Time time,
			DataSpace ds, int numOfClasses) {
		...
		extension[num - 1] = (ds.getMaxX() - ds.getMinX() + ds.getMaxY() - ds
				.getMinY()) / 5;
	...
	public int computeNewExternalObjectClass(int time) {
		/*
		 * HAHAHAHAHA Below are all not needed. 
		 * int value = Math.abs(random.nextInt())%PORTIONSUM; int[] p = portion;
		 * if (time == 0) p = beginPortion; for (int i=0; i<num; i++) if
		 * (value-p[i]<0) return i; else value -= p[i];
		 */
		return num - 1;
	}
